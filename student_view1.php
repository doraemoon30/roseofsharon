...
const qProgress=document.getElementById('qProgress'), owlCenter=document.getElementById('owl-center'),
      letsAnswer=document.getElementById('letsAnswerText'), feedbackEl=document.getElementById('feedback'),
      crying=document.getElementById('crying-smiley'), redFlash=document.getElementById('red-flash'),
      fsBtn=document.getElementById('fullscreenBtn'), videoEl=document.getElementById('storybookVideo');

function openFullscreen(){const el=document.documentElement;(el.requestFullscreen||el.webkitRequestFullscreen||el.msRequestFullscreen)?.call(el);if(fsBtn)fsBtn.style.display='none';}
function showBalloons(count=80){for(let i=0;i<count;i++){const b=document.createElement('div');b.className='balloon';b.style.left=(Math.random()*100)+'vw';b.style.top=(Math.random()*100)+'vh';b.style.background=`hsl(${Math.random()*360},80%,60%)`;b.style.setProperty('--scale',(0.6+Math.random()*0.9).toString());b.style.setProperty('--dur',(6+Math.random()*4)+'s');b.style.setProperty('--delay',(Math.random()*0.8)+'s');const dx=(Math.random()*300-150),dy=-(60+Math.random()*80)+'vh';b.style.setProperty('--dx',dx+'px');b.style.setProperty('--dy',dy);document.body.appendChild(b);const ttl=(parseFloat(b.style.getPropertyValue('--dur'))+parseFloat(b.style.getPropertyValue('--delay')))*1000+500;setTimeout(()=>b.remove(),ttl);}}
function bigConfetti(duration=ANIM_DURATION){const end=Date.now()+duration;(function frame(){confetti({particleCount:20,spread:90,startVelocity:55,origin:{x:Math.random(),y:Math.random()*0.4}});if(Date.now()<end)requestAnimationFrame(frame);})(); }
function showCorrect(){feedbackEl.textContent='✔️';document.getElementById('correctSound').play().catch(()=>{});showBalloons(100);bigConfetti(ANIM_DURATION);}
function rainTears(durationMs=10000){const start=Date.now(),spawnEveryMs=90,maxLifeMs=7000;const spawn=()=>{if(Date.now()-start>durationMs)return;const t=document.createElement('div');t.className='tear';t.textContent='💧';t.style.left=(2+Math.random()*96)+'vw';const driftPx=(Math.random()*120-60),scale=0.8+Math.random()*0.8,durSec=3.2+Math.random()*3.0,fontPx=22+Math.random()*26;t.style.setProperty('--drift',driftPx+'px');t.style.setProperty('--scale',scale.toFixed(2));t.style.setProperty('--dur',durSec.toFixed(2)+'s');t.style.fontSize=fontPx+'px';document.body.appendChild(t);setTimeout(()=>t.remove(),Math.min((durSec*1000)+200,maxLifeMs));setTimeout(spawn,spawnEveryMs);};spawn();}
function showWrong(){feedbackEl.textContent='❌';feedbackEl.style.fontSize='8rem';const audio=document.getElementById('wrongSound');audio&&audio.play().catch(()=>{});$(redFlash).stop(true,true).fadeIn(150).fadeOut(600);if(showWrong._cryTimer)clearTimeout(showWrong._cryTimer);$(crying).stop(true,true).fadeIn(150);showWrong._cryTimer=setTimeout(()=>$(crying).fadeOut(300),10000);rainTears(10000);}
function clearFeedback(){feedbackEl.textContent='';feedbackEl.style.fontSize='';}
...
function tryAutoPlayMuted(v){if(!v)return;v.muted=true;v.play().catch(()=>{});}
const v=document.getElementById('storybookVideo');
if(v){tryAutoPlayMuted(v);v.addEventListener('ended',startFlySequence);}else{startFlySequence();}
</script>
</body>
</html>
